...
BEGIN
    -- Verificar se houve altera��o no status
    IF UPDATE(ativo)
    BEGIN
        ...
        BEGIN
            -- Lan�ar erro e abortar a atualiza��o
            RAISERROR('N�o � poss�vel desativar o cliente devido a faturas em aberto.', 16, 1)
        ...
        BEGIN
            -- Lan�ar erro e abortar a atualiza��o
            RAISERROR('N�o � poss�vel desativar o cliente devido a investimentos ativos.', 16, 1)
            ROLLBACK TRANSACTION
            RETURN
        END
        
        -- Verificar se existem empr�stimos ativos
        IF EXISTS (SELECT 1 FROM [Emprestimo] WHERE id_conta IN (SELECT id_conta FROM inserted) AND ativo = 1)
        BEGIN
            -- Lan�ar erro e abortar a atualiza��o
            RAISERROR('N�o � poss�vel desativar o cliente devido a empr�stimos ativos.', 16, 1)
            ROLLBACK TRANSACTION
            RETURN
        END
        
        -- Verificar se existem cart�es de cr�dito ativos
        IF EXISTS (SELECT 1 FROM [Cartao] WHERE id_conta IN (SELECT id_conta FROM inserted) AND ativo = 1)
        BEGIN
            -- Lan�ar erro e abortar a atualiza��o
            RAISERROR('N�o � poss�vel desativar o cliente devido a cart�es de cr�dito ativos.', 16, 1)
            ROLLBACK TRANSACTION
            RETURN
        END
    END
...
BEGIN
    -- Definir os valores das moedas em rela��o ao BRL
    DECLARE @valor_dolar numeric(14,2) = 4.77
    DECLARE @valor_euro numeric(14,2) = 5.22

    -- Vari�veis para armazenar os valores da inser��o
    ...
    -- Obter o id_cliente associado � conta e verificar se a conta est� ativa
    ...
    -- Verificar se a conta est� desativada
    IF @ativo = 0
    BEGIN
        RAISERROR('A conta est� desativada. N�o � poss�vel realizar o c�mbio.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END

    -- Verificar se a conta possui saldo suficiente para a convers�o
    ...
    BEGIN
        RAISERROR('A conta n�o possui saldo suficiente para realizar o c�mbio.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END

    -- BRLizar a convers�o de moeda
	...
	ELSE
	BEGIN
		RAISERROR('Moeda n�o reconhecida', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
	END
END
...
BEGIN
    ...
    -- Verificar se a conta est� ativa para realizar a opera��o
    IF EXISTS (
        SELECT 1
        FROM [inserted] i
    ...
    BEGIN
        -- Lan�ar erro e abortar a atualiza��o
        RAISERROR('N�o � poss�vel realizar o investimento em uma conta desativada.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END
    
    -- Verificar se o investimento est� sendo desativado
    ...
    -- Verificar se h� saldo suficiente para o investimento
    IF EXISTS (
        SELECT 1
        FROM [inserted] i
    ...
    BEGIN
        -- Lan�ar erro e abortar a atualiza��o
        RAISERROR('N�o h� saldo suficiente na conta para realizar o investimento.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END
    ...
    -- Atualizar o valor_rendimento com base no tipo de investimento e diferen�a de datas
    UPDATE i
...
BEGIN
    ...
    BEGIN
        RAISERROR('A conta est� inativa. Opera��o de empr�stimo n�o permitida.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END

    -- Verificar se h� saldo no limite de empr�stimo na conta do usu�rio
    IF EXISTS (
        SELECT 1 FROM inserted i
        INNER JOIN Conta c ON i.id_conta = c.id_conta
    ...
    BEGIN
        RAISERROR('A conta n�o possui saldo suficiente no limite de empr�stimo.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END

    -- Verificar se o saldo_real da conta � maior ou igual a 0
    IF EXISTS (
        SELECT 1 FROM inserted i
        INNER JOIN Conta c ON i.id_conta = c.id_conta
    ...
    BEGIN
        RAISERROR('A conta possui saldo negativo. Opera��o de empr�stimo n�o permitida.', 16, 1)
        ROLLBACK TRANSACTION
        RETURN
    END

    -- Armazenar valores necess�rios em vari�veis
    ...
    -- Calcular o saldo devedor considerando a taxa de juros e a diferen�a de minutos
    ...
    -- Atualizar o limite de empr�stimo na conta do usu�rio
    UPDATE c
    SET c.lim_emprest = c.lim_emprest - @valor_emprest
    FROM Conta c
    INNER JOIN inserted i ON c.id_conta = i.id_conta

    -- Desativar empr�stimos vencidos
    UPDATE e
    SET e.ativo = 0
    FROM Emprestimo e
    WHERE e.data_venc < GETDATE()

    -- Verificar se h� empr�stimos vencidos
    IF EXISTS (
        SELECT 1 FROM Emprestimo
        WHERE data_venc < GETDATE()
    )
    BEGIN
        -- Mover empr�stimos vencidos para a tabela Emprestimodesativado (apenas se ainda n�o estiverem na tabela)
...
BEGIN
    ...
    -- Obter os valores relevantes do empr�stimo desativado inserido
    ...
    -- Verificar se h� uma fatura em aberto para a conta associada ao empr�stimo desativado
    IF EXISTS (SELECT 1 FROM Fatura WHERE id_conta = @id_conta AND ativo = 1)
    BEGIN
        -- Atualizar o saldo da fatura com base no valor do saldo_devedor do empr�stimo
        UPDATE Fatura
    ...
    ELSE
    BEGIN
        -- Criar uma nova fatura com o valor_emprest igual ao saldo_devedor do empr�stimo
...
BEGIN
    ...
    -- Verificar se a conta pagadora ou benefici�ria est� ativa
    ...
    BEGIN
        ...
        RAISERROR('A conta pagadora ou benefici�ria n�o est� ativa. A transa��o foi reprovada.', 16, 1);
        --ROLLBACK;
        RETURN;
    END;
    
    -- Verificar saldo e efetuar transfer�ncia
    ...
    BEGIN
        ...
        BEGIN
            -- Valor da transa��o � maior do que o saldo do pagador, reprovar a transa��o
            UPDATE Transacao
            SET status = 'Reprovado'
            WHERE id_trans = (SELECT id_trans FROM inserted);
            
            RAISERROR('Saldo insuficiente na conta do pagador. A transa��o foi reprovada.', 16, 1);
            --ROLLBACK;
            RETURN;
        END;
        ...
        -- Incrementar saldo do benefici�rio
        UPDATE Conta
        SET saldo_real = saldo_real + @ValorTransacao
        WHERE id_conta = @IdClienteBeneficiario;
        
        -- Atualizar status da transa��o
        UPDATE Transacao
        SET status = 'Aprovado'
        WHERE id_trans = (SELECT id_trans FROM inserted);
    END;
    
    -- Realizar opera��o de Dep�sito
    ...
    BEGIN
        ...
        BEGIN
            ...
            -- Reduzir limite de cr�dito do pagador
            UPDATE Conta
        ...
        ELSE
        BEGIN
            -- Limite de cr�dito insuficiente, reprovar a transa��o
            UPDATE Transacao
            SET status = 'Reprovado'
            WHERE id_trans = (SELECT id_trans FROM inserted);
            
            RAISERROR('Limite de cr�dito insuficiente na conta do pagador. A transa��o foi reprovada.', 16, 1);
            --ROLLBACK;
            RETURN;
        END;
    END;

    -- Verificar se h� uma fatura ativa
    ...
    BEGIN
        ...
        BEGIN
            -- Adicionar valor da transa��o � fatura ativa
            UPDATE Fatura
        ...
        ELSE
        BEGIN
            -- Criar nova fatura com o valor da transa��o
            INSERT INTO Fatura (ativo, id_conta, valor, valor_emprest, dt_venc)
...
BEGIN
    ...
    BEGIN
        ...
        -- Verificar se o ID da fatura j� existe na tabela Faturadesativada
